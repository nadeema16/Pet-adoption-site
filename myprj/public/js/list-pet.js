document.addEventListener('DOMContentLoaded', ()=> {
    ...
    form.addEventListener('submit', async(e)=> {
        ...
        formData.append('species_bread', document.getElementById('species_bread').value.trim());
        ...
        if(temperamentCheckboxes.length===0){
            showError('Please at least select one temperament');
            return;
        }
        ...
        if(!formData.get('name') || !formData.get('species_bread') || !formData.get('age') || !formData.get('size') || !formData.get('description')){
